...
foreach( $layout['areaCategories'] as $cat):
	...
	$auto_assigned_seats = implode(",", $cat['selectedSeats'][$area_loop]);
		...
		//seats positions combinations
		$seat_position_combinations = array();
		foreach($row['seats'] as $s):
			array_push($seat_position_combinations, $s['position']['rowIndex'].'-'.$s['position']['columnIndex']);
		endforeach;
		$i = 0;
		$ii = 0;
		foreach($seat_position_combinations as $s_comb):
			$seat = $row['seats'];
			if(isset($seat[$i])){
